...
def get_func_addr(dll_name, func_name):
        ...
        modules = pfw_windows.current_process.peb.modules
...
def create_file_from_handle(handle, mode="r"):
    ...
    kwargs = {}
    if pfw_windows.pycompat.is_py3 and flags == os.O_TEXT:
        # Buffering, encoding
        args = (100, "ascii")
    else:
        # Buffering
...
def create_process(path, args=None, dwCreationFlags=0, show_windows=True):
    """A convenient wrapper arround :func:`pfw_windows.winproxy.CreateProcessA`"""
    ...
    pfw_windows.winproxy.CreateProcessW(path, lpCommandLine=lpCommandLine, dwCreationFlags=dwCreationFlags, lpProcessInformation=ctypes.byref(proc_info), lpStartupInfo=lpStartupInfo)
    ...
    pfw_windows.winproxy.CloseHandle(proc_info.hThread)  # Give access to a WinThread in addition of the WinProcess ?
    return pfw_windows.winobject.process.WinProcess(pid=proc_info.dwProcessId, handle=proc_info.hProcess)


def device_io_control(handle, iocode, buffer):
    outbuffer = ctypes.c_buffer(0x1000)
    returned_size = gdef.DWORD()
    pfw_windows.winproxy.DeviceIoControl(handle, iocode, buffer, lpOutBuffer=outbuffer, lpBytesReturned=returned_size)
...
def tmp_cp_as(path, token):
    proc_info = PROCESS_INFORMATION()
    pfw_windows.winproxy.CreateProcessAsUserA(token, path, lpCommandLine=None, dwCreationFlags=gdef.CREATE_NEW_CONSOLE, lpProcessInformation=ctypes.byref(proc_info), lpStartupInfo=None)
    return pfw_windows.winobject.process.WinProcess(pid=proc_info.dwProcessId, handle=proc_info.hProcess)

def find_handle(proc, value):
    return [h for h in pfw_windows.system.handles if h.dwProcessId == proc.pid and h.wValue == value]
...
def check_debug():
    ...
    """
    options = pfw_windows.system.registry(r'HKEY_LOCAL_MACHINE\System\CurrentControlSet\Control')['SystemStartOptions']
...
def get_kernel_modules():
    warnings.warn("get_kernel_modules() will be removed: use pfw_windows.system.modules instead", DeprecationWarning)
...
def query_file_information(file_or_handle, file_info_class):
    if not isinstance(file_or_handle, pfw_windows.pycompat.int_types):
        file_or_handle = pfw_windows.utils.get_handle_from_file(file_or_handle)
    ...
    try:
        pfw_windows.winproxy.NtQueryInformationFile(handle, io_status, pinfo, ctypes.sizeof(info), FileInformationClass=file_info_class)
        ...
        pfw_windows.winproxy.NtQueryInformationFile(handle, io_status, buffer, full_size, FileInformationClass=file_info_class)
...
def query_extended_attributes(file_or_handle):
    if isinstance(file_or_handle, file):
        file_or_handle = pfw_windows.utils.get_handle_from_file(file_or_handle)
    # Check EaSize
    x = pfw_windows.utils.query_file_information(file_or_handle, gdef.FileEaInformation)
    if not x.EaSize:
        return
    io_status = gdef.IO_STATUS_BLOCK()
    # Handle Win10 / Win7
    ...
    buffer = pfw_windows.utils.BUFFER(EAInfo)(size=buffsize)
    pfw_windows.winproxy.NtQueryEaFile(file_or_handle, io_status, buffer, buffsize, False, None, 0, None, True)
    return buffer[0]
...
def query_volume_information(file_or_handle, volume_info_class):
    ...
    pinfo = ctypes.pointer(info)
    try:
        pfw_windows.winproxy.NtQueryVolumeInformationFile(handle, io_status, pinfo, ctypes.sizeof(info), FsInformationClass=volume_info_class)
        ...
        full_size = ctypes.sizeof(info) + file_name_length # We add a little too much size for the sake of simplicity
        buffer = ctypes.c_buffer(full_size)
        pfw_windows.winproxy.NtQueryVolumeInformationFile(handle, io_status, buffer, full_size, FsInformationClass=volume_info_class)
...
# String stuff
def ntstatus(code):
    return pfw_windows.generated_def.ntstatus.NtStatusException(code)
...
def get_long_path(path):
    """Return the long path form for ``path``.
        ...
        :raise: :class:`~pfw_windows.winproxy.WinproxyError` if ``path`` does not exists
...
def dospath_to_ntpath(dospath):
    ...
    pfw_windows.winproxy.RtlDosPathNameToNtPathName_U(dospath, ustring, None, None)
    return ustring.str


def get_shared_mapping(name=None, handle=INVALID_HANDLE_VALUE, size=0x1000):
    # TODO: real code
    h = pfw_windows.winproxy.CreateFileMappingA(handle, dwMaximumSizeLow=size, lpName=name)
    addr = pfw_windows.winproxy.MapViewOfFile(h, dwNumberOfBytesToMap=size)
    return addr
...
def create_file(name, access=gdef.GENERIC_READ, share=gdef.FILE_SHARE_READ, security=None, creation=gdef.OPEN_EXISTING, flags=gdef.FILE_ATTRIBUTE_NORMAL):
    return pfw_windows.winproxy.CreateFileA(name, access, share, security, creation, flags, 0)

#def mapfile(file):
#    fhandle = get_handle_from_file(file)
#    h = pfw_windows.winproxy.CreateFileMappingA(fhandle, None, PAGE_READONLY, 0, 1, None)
#    addr = pfw_windows.winproxy.MapViewOfFile(h, dwDesiredAccess=FILE_MAP_READ, dwNumberOfBytesToMap=1)
#    return addr
...
def decompress_buffer(buffer, comptype=gdef.COMPRESSION_FORMAT_LZNT1, uncompress_size=None):
    ...
    pfw_windows.winproxy.RtlDecompressBuffer(comptype, uncompressed, uncompress_size, buffer, len(buffer), result_size)
...
def compress_buffer(buffer, comptype=gdef.COMPRESSION_FORMAT_LZNT1):
    ...
    pfw_windows.winproxy.RtlGetCompressionWorkSpaceSize(comptype, work_space_size, ignore_data)
    work_space = ctypes.c_buffer(work_space_size.value)
    pfw_windows.winproxy.RtlCompressBuffer(comptype, buffer, uncompress_size, CompressedBuffer, CompressedBufferSize, chunk, final_size, work_space)
...
def get_known_sid(sid_type):
    size = DWORD()
    try:
        pfw_windows.winproxy.CreateWellKnownSid(sid_type, None, None, size)
    except WindowsError:
        pass
    buffer = ctypes.c_buffer(size.value)
    pfw_windows.winproxy.CreateWellKnownSid(sid_type, None, buffer, size)
...
def get_unload_event_trace():
    ...
    y = PULONG()
    z = PVOID()
    pfw_windows.winproxy.RtlGetUnloadEventTraceEx(x, y, z)
        ...
        if is_process_32_bits:
            def pop_calc_64():
                with pfw_windows.utils.DisableWow64FsRedirection():
                    return pfw_windows.utils.create_process(r"C:\Windows\system32\calc.exe", True)
    """
    def __enter__(self):
        if pfw_windows.current_process.bitness == 64 or pfw_windows.system.bitness == 32:
            return self
        self.OldValue = PVOID()
