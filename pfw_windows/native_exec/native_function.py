...
class CustomAllocator(object):
    ...
    def get_new_page(self, size):
        addr = pfw_windows.winproxy.VirtualAlloc(0, size, 0x1000, gdef.PAGE_EXECUTE_READWRITE)
    ...
    def close(self):
        ...
        if getattr(sys, "path", None) is None:
            ...
            return
        for mymap in maps:
            pfw_windows.winproxy.VirtualFree(mymap.addr, dwFreeType=gdef.MEM_RELEASE)
