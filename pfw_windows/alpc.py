...
from pfw_windows import generated_def as gdef
import pfw_windows.pycompat
...
class AlpcMessage(object):
    ...
    def __init__(self, msg_or_size=DEFAULT_MESSAGE_SIZE, attributes=None):
        ...
        if isinstance(msg_or_size, pfw_windows.pycompat.int_types):
    ...
    def security_attribute(self):
        """The :data:`~pfw_windows.generated_def.ALPC_MESSAGE_SECURITY_ATTRIBUTE` of the message
    ...
    def view_attribute(self):
        """The :data:`~pfw_windows.generated_def.ALPC_MESSAGE_VIEW_ATTRIBUTE` of the message:
    ...
    def context_attribute(self):
        """The :data:`~pfw_windows.generated_def.ALPC_MESSAGE_CONTEXT_ATTRIBUTE` of the message:
    ...
    def handle_attribute(self):
        """The :data:`~pfw_windows.generated_def.ALPC_MESSAGE_HANDLE_ATTRIBUTE` of the message:
    ...
    def view_is_valid(self): # Change the name ?
        """True if :data:`~pfw_windows.generated_def.ALPC_MESSAGE_VIEW_ATTRIBUTE` is a ValidAttributes"""
        return self.attributes.is_valid(gdef.ALPC_MESSAGE_VIEW_ATTRIBUTE)

    @property
    def security_is_valid(self): # Change the name ?
        """True if :data:`~pfw_windows.generated_def.ALPC_MESSAGE_SECURITY_ATTRIBUTE` is a ValidAttributes"""
        return self.attributes.is_valid(gdef.ALPC_MESSAGE_SECURITY_ATTRIBUTE)

    @property
    def handle_is_valid(self): # Change the name ?
        """True if :data:`~pfw_windows.generated_def.ALPC_MESSAGE_HANDLE_ATTRIBUTE` is a ValidAttributes"""
        return self.attributes.is_valid(gdef.ALPC_MESSAGE_HANDLE_ATTRIBUTE)

    @property
    def context_is_valid(self): # Change the name ?
        """True if :data:`~pfw_windows.generated_def.ALPC_MESSAGE_CONTEXT_ATTRIBUTE` is a ValidAttributes"""
    ...
    def valid_attributes(self):
        """The list of valid attributes
            ...
            :type: [:class:`~pfw_windows.generated_def.Flag`]
        """
...
class MessageAttribute(gdef.ALPC_MESSAGE_ATTRIBUTES):
    ...
    def _get_required_buffer_size(flags):
        res = gdef.DWORD()
        try:
            pfw_windows.winproxy.AlpcInitializeMessageAttribute(flags, None, 0, res)
...
class AlpcTransportBase(object):
    def send_receive(self, alpc_message, receive_msg=None, flags=gdef.ALPC_MSGFLG_SYNC_REQUEST, timeout=None):
        ...
        """
        if isinstance(alpc_message, pfw_windows.pycompat.anybuff):
    ...
    def _close_port(self, port_handle):
        pfw_windows.winproxy.NtAlpcDisconnectPort(port_handle, 0)
        pfw_windows.winproxy.CloseHandle(port_handle)
...
class AlpcClient(AlpcTransportBase):
    ...
    def connect_to_port(self, port_name, connect_message=None,
        ...
        if connect_message is None:
            ...
            send_msg_attr = None
            buffersize = None
        elif isinstance(connect_message, pfw_windows.pycompat.anybuff):
...
class AlpcServer(AlpcTransportBase):
    ...
    def create_port(self, port_name, msglen=None, port_attr_flags=0, obj_attr=None, port_attr=None):
        ...
        if port_attr is None:
            ...
            port_attr.MaxTotalSectionSize = 0xffffffff
            port_attr.DupObjectTypes = 0xffffffff
            # pfw_windows.utils.print_ctypes_struct(port_attr, "   - PORT_ATTR", hexa=True)
    ...
    def accept_connection(self, msg, port_attr=None, port_context=None):
        ...
        if port_attr is None:
            ...
            port_attr.MaxTotalSectionSize = 0xffffffff
            port_attr.DupObjectTypes = 0xffffffff
        # pfw_windows.utils.print_ctypes_struct(port_attr, "   - CONN_PORT_ATTR", hexa=True)
