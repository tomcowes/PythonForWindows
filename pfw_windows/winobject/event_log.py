...
from pfw_windows.pycompat import int_types, basestring
...
def queryinfo(handle, propertyid):
    ...
    res = gdef.DWORD()
    pfw_windows.winproxy.EvtGetLogInfo(handle, propertyid, size, evt, res)
    return evt
...
def arrayproperty(handle, property, index, flags=0):
    ...
    evt = ImprovedEVT_VARIANT.from_buffer(buffer)
    res = gdef.DWORD()
    pfw_windows.winproxy.EvtGetObjectArrayProperty(handle, property, index, flags, size, evt, res)
    return evt
...
class EvtHandle(gdef.EVT_HANDLE):
    ...
    _close_function = pfw_windows.winproxy.EvtClose
...
class EvtQuery(EvtHandle):
    ...
    def __next__(self):
        ...
        try:
            ...
            ret = gdef.DWORD()
            pfw_windows.winproxy.EvtNext(self, 1, event, self.timeout, 0, ret)
    ...
    def seek(self, position, seek_flags=None):
        ...
        if seek_flags is None:
            if position >= 0:
                ...
                position += 1
        pfw_windows.winproxy.EvtSeek(self, position, 0, 0, seek_flags)
...
class EvtEvent(EvtHandle):
    ...
    def render(self, ctx, rtype):
        ...
        try:
            pfw_windows.winproxy.EvtRender(ctx, self, rtype, size, buffer, rsize, elementnb)
        except WindowsError as e:
    ...
    def get_values(self, values, flags=gdef.EvtRenderContextValues):
        ...
        ctx = pfw_windows.winproxy.EvtCreateRenderContext(nbelt, pwstr_rarray, gdef.EvtRenderContextValues)
        result = self.render(ctx, gdef.EvtRenderEventValues)
        pfw_windows.winproxy.EvtClose(ctx)
        return [r.value for r in result]

    def system_values(self): # POC: use this for all @property based on system data ?
        ctx = pfw_windows.winproxy.EvtCreateRenderContext(0, None, gdef.EvtRenderContextSystem)
        result = self.render(ctx, gdef.EvtRenderEventValues)
        pfw_windows.winproxy.EvtClose(ctx)
        return [r.value for r in result]
    ...
    def event_values(self):
        ...
        ctx = pfw_windows.winproxy.EvtCreateRenderContext(0, None, gdef.EvtRenderContextUser)
        result = self.render(ctx, gdef.EvtRenderEventValues)
        pfw_windows.winproxy.EvtClose(ctx)
        return [r.value for r in result]
    ...
    def date(self):
        ...
        return pfw_windows.utils.datetime_from_filetime(self.time_created)

    def __repr__(self):
        creation_time = pfw_windows.utils.datetime_from_filetime(self.time_created)
    ...
    def from_value(cls, value, vtype=None):
        if vtype is None:
            ...
            if isinstance(value, int_types):
                ...
                value = pfw_windows.utils.BUFFER(gdef.BYTE).from_buffer_copy(value)
            else:
...
class PublisherMetadata(EvtHandle):
    ...
    def events_metadata(self):
        ...
        with ClosingEvtHandle(eh):
            while True:
                try:
                    nh = pfw_windows.winproxy.EvtNextEventMetadata(eh, 0)
...
class PropertyArray(gdef.EVT_OBJECT_ARRAY_PROPERTY_HANDLE):
    ...
    def size(self):
        ...
        pfw_windows.winproxy.EvtGetObjectArraySize(self, array_size)
...
class EvtlogManager(object):
    ...
    def is_implemented(self):
        ...
        """
        return pfw_windows.winproxy.is_implemented(pfw_windows.winproxy.EvtQuery)

    @property
    def channels(self):
        h = pfw_windows.winproxy.EvtOpenChannelEnum(None, 0)
        size = 0x1000
        ...
        with ClosingEvtHandle(h):
            while True:
                try:
                    pfw_windows.winproxy.EvtNextChannelPath(h, size, buffer, ressize)
                except WindowsError as e:
                    if e.winerror != gdef.ERROR_NO_MORE_ITEMS:
                        raise
                    return
    ...
    @property
    def publishers(self):
        h = pfw_windows.winproxy.EvtOpenPublisherEnum(None, 0)
        size = 0x1000
        buffer = ctypes.create_unicode_buffer(size)
        ressize = gdef.DWORD()
        with ClosingEvtHandle(h):
            while True:
                try:
                    pfw_windows.winproxy.EvtNextPublisherId(h, size, buffer, ressize)
                except WindowsError as e:
                    if e.winerror != gdef.ERROR_NO_MORE_ITEMS:
                        raise
                    return
