...
class AbstractAction(object):
    ...
    :type: :class:`~pfw_windows.generated_def.winstructs.TASK_ACTION_TYPE`
    """
    type = generate_simple_getter("get_Type", gdef.TASK_ACTION_TYPE, doc=type_doc)

    id_doc = """The action id

    :type: :class:`~pfw_windows.generated_def.winstructs.BSTR`
    """
...
class ExecAction(gdef.IExecAction, AbstractAction):
    """Represent an action of type
        :data:`~pfw_windows.generated_def.winstructs._TASK_ACTION_TYPE.TASK_ACTION_EXEC`"""
...
class ComHandlerAction(gdef.IComHandlerAction, AbstractAction):
    """Represent an action of type
        :data:`~pfw_windows.generated_def.winstructs._TASK_ACTION_TYPE.TASK_ACTION_COM_HANDLER`"""
...
class Trigger(gdef.ITrigger):
    ...
    :type: :class:`~pfw_windows.generated_def.winstructs.TASK_TRIGGER_TYPE2`
    """
...
class TaskRegistrationInfo(gdef.IRegistrationInfo):
    ...
    sddl = generate_simple_getter("get_SecurityDescriptor", pfw_windows.com.Variant)
    ...
    def security_descriptor(self):
        ...
        if not sddl:
            return None
        return pfw_windows.security.SecurityDescriptor.from_string(sddl)
...
class TaskPrincipal(gdef.IPrincipal):
    ...
    :type: :class:`~pfw_windows.generated_def.winstructs.TASK_RUNLEVEL_TYPE`
    """
    logon_type = generate_simple_getter("get_LogonType", gdef.TASK_LOGON_TYPE)
    """ logon method that is required to run the task

    :type: :class:`~pfw_windows.generated_def.winstructs.TASK_LOGON_TYPE`
    """
...
class Task(gdef.IRegisteredTask):
    ...
    """The state of the task
        ...
        :type: :class:`~pfw_windows.generated_def.winstructs.TASK_STATE`
    """
...
class TaskCollection(gdef.IRegisteredTaskCollection, TaskCollectionType):
    ITEM_TYPE = Task
    def get_index(self, index):
        vindex = pfw_windows.com.Variant()
...
# pfw_windows.com.create_instance(clsid_task_scheduler, x)
# x.connect()
# folder = x.folder("\\")
# assert folder.value
# tasks = folder.tasks
