...
class QualifierSet(gdef.IWbemQualifierSet):
    def get_variant(self, name):
        """Retrieve the value of property ``name`` as a :class:`~pfw_windows.com.Variant`

        :return: :class:`~pfw_windows.com.Variant`
        """
        if not isinstance(name, basestring):
            nametype = type(name).__name__
            raise TypeError("WmiObject attributes name must be str, not <{0}>".format(nametype))
        variant_res = pfw_windows.com.Variant()
    ...
    def names(self):
        res = POINTER(pfw_windows.com.SafeArray)()
...
class WmiObject(gdef.IWbemClassObject, WmiComInterface):
    ...
    def put(self, name, value):
        ...
        variant_value = pfw_windows.com.Variant(value)
...
class WmiNamespace(gdef.IWbemServices, WmiComInterface):
    ...
    #CLSID_WbemAdministrativeLocator_IID = pfw_windows.com.IID.from_string('CB8555CC-9128-11D1-AD9B-00C04FD8FDFF')
    WbemLocator_CLSID = pfw_windows.com.IID.from_string('4590F811-1D3A-11D0-891F-00AA004B2E24')
    ...
    def connect(cls, namespace, user=None, password=None):
        ...
        locator = WmiLocator()
        pfw_windows.com.create_instance(cls.WbemLocator_CLSID, locator)
    ...
    def create_instance_enum(self, clsname, flags=DEFAULT_ENUM_FLAGS, deep=True):
        ...
        Example:
            >>> pfw_windows.system.wmi["root\\subscription"].create_instance_enum("__EventConsumer", deep=False).all()
            []
            >>> pfw_windows.system.wmi["root\\subscription"].create_instance_enum("__EventConsumer", deep=True).all()
...
class WmiManager(dict):
    ...
    Example:
        >>> pfw_windows.system.wmi["root\\SecurityCenter2"]
