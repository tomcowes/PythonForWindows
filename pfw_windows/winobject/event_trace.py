...
class EventTraceProperties(gdef.EVENT_TRACE_PROPERTIES):
    ...
    def create(cls):
        ...
        buff = pfw_windows.utils.BUFFER(cls)(size=cls.FULL_SIZE)
    ...
    def get_logfilename(self):
        ...
        return pfw_windows.current_process.read_string(ctypes.addressof(self) + self.LogFileNameOffset)

    def set_logfilename(self, filename):
        assert self.LogFileNameOffset
        if not filename.endswith("\x00"):
            filename += "\x00"
        return pfw_windows.current_process.write_memory(ctypes.addressof(self) + self.LogFileNameOffset, filename)

    logfile = property(get_logfilename, set_logfilename) #: The logfile associated with the session

    def get_logger_name(self):
        assert self.LoggerNameOffset
        return pfw_windows.current_process.read_string(ctypes.addressof(self) + self.LoggerNameOffset)
    ...
    def set_logfilename(self, filename):
        ...
        if not filename.endswith("\x00"):
            filename += "\x00"
        return pfw_windows.current_process.write_memory(ctypes.addressof(self) + self.LoggerNameOffset, filename)
...
class CtxProcess(object):
    ...
    def _get_time(self):
        now = gdef.FILETIME()
        pfw_windows.winproxy.GetSystemTimeAsFileTime(now)
        return now
...
class EtwTrace(object):
    ...
    def __init__(self, name, logfile=None, guid=None):
        self.name = pfw_windows.pycompat.raw_encode(name) #: The name of the trace
    ...
    def exists(self):
        ...
        try:
            pfw_windows.winproxy.ControlTraceA(self.handle, self.name, prop, gdef.EVENT_TRACE_CONTROL_QUERY)
    ...
    def start(self, flags=0, mode=0):
        ...
        pfw_windows.winproxy.StartTraceA(handle, self.name, prop)
        if not self.guid:
    ...
    def stop(self, soft=False): # Change name
        ...
        prop = EventTraceProperties.create()
        try:
            pfw_windows.winproxy.ControlTraceA(0, self.name, prop, gdef.EVENT_TRACE_CONTROL_STOP)
        except WindowsError as e:
    ...
    def flush(self):
        """Flush the trace"""
        prop = EventTraceProperties.create()
        pfw_windows.winproxy.ControlTraceA(0, self.name, prop, gdef.EVENT_TRACE_CONTROL_FLUSH)
    ...
    def enable(self, guid, flags=0xff, level=0xff):
        ...
        if isinstance(guid, basestring):
            guid = gdef.GUID.from_string(guid)
        return pfw_windows.winproxy.EnableTrace(1, flags, level, guid, self.handle) # EnableTraceEx ?
    ...
    def enable_ex(self, guid, flags=0xff, level=0xff, any_keyword = 0xffffffff, all_keyword=0x00):
        ...
        return pfw_windows.winproxy.EnableTraceEx2(self.handle, guid, EVENT_CONTROL_CODE_ENABLE_PROVIDER, level , any_keyword, all_keyword, 0, None)
    ...
    def process(self, callback, begin=None, end=None, context=None):
        ...
        if end == "now":
            ...
            pfw_windows.winproxy.GetSystemTimeAsFileTime(end)
            pfw_windows.utils.sprint(end)

        logfile = gdef.EVENT_TRACE_LOGFILEW()
        logfile.LoggerName = pfw_windows.pycompat.raw_decode(self.name)
        ...
        r = pfw_windows.winproxy.OpenTraceW(logfile)
        rh = gdef.TRACEHANDLE(r)
        return pfw_windows.winproxy.ProcessTrace(rh, 1, begin, end)
...
class TraceProvider(object):
    ...
    def infos(self):
        ...
        try:
            pfw_windows.winproxy.EnumerateTraceGuidsEx(gdef.TraceGuidQueryInfo, self.guid, ctypes.sizeof(self.guid), info_buffer, ctypes.sizeof(info_buffer), size)
        except WindowsError as e:
...
class TraceProviderInstanceInfo(gdef.TRACE_PROVIDER_INSTANCE_INFO):
    ...
    def sessions(self):
        ...
        :type: [:class:`~pfw_windows.generated_def.winstructs.TRACE_ENABLE_INFO`] -- A list of session
        """
        return [x for x in self._instance_generator()]
...
class EtwManager(object):
    ...
    def sessions(self):
        ...
        count = gdef.DWORD()
        pfw_windows.winproxy.QueryAllTracesA(tarray, MAX_ETW_SESSIONS, count)
    ...
    def providers(self):
        ...
        """
        buffer = pfw_windows.utils.BUFFER(gdef.GUID, 0x1000)()
        size = gdef.DWORD()
        pfw_windows.winproxy.EnumerateTraceGuidsEx(gdef.TraceGuidQueryList, None, 0, buffer, buffer.real_size, size)
