...
class BaseSystemHandle(object):
    ...
    @pfw_windows.utils.fixedpropety
    def process(self):
        """The process possessing the handle

        :type: :class:`WinProcess <pfw_windows.winobject.process.WinProcess>`"""
        # "TODO: something smart ? :D"
        # return [p for p in pfw_windows.system.processes if p.pid == self.dwProcessId][0]
        return pfw_windows.WinProcess(pid=self.dwProcessId)
    ...
    def local_handle(self):
        ...
        :type: :class:`int`"""
        if self.dwProcessId == pfw_windows.current_process.pid:
            return self.wValue
        res = gdef.HANDLE()
        winproxy.DuplicateHandle(self.process.handle, self.wValue, pfw_windows.current_process.handle, ctypes.byref(res), dwOptions=gdef.DUPLICATE_SAME_ACCESS)
        return res.value
    ...
    def description_Process(self):
        proc = pfw_windows.WinProcess(handle=self.wValue)
        res = str(proc)
        del proc._handle
        return res

    def description_Thread(self):
        thread = pfw_windows.WinThread(handle=self.wValue)
        res = str(thread)
        del thread._handle
        return res
...
def enumerate_handles():
    ...
    tmp_buffer = pfw_windows.utils.BUFFER(gdef.SYSTEM_HANDLE_INFORMATION)()
    try:
    ...
    buf = pfw_windows.utils.BUFFER(gdef.SYSTEM_HANDLE_INFORMATION)(size=size)
    size_needed.value = 0
    winproxy.NtQuerySystemInformation(gdef.SystemHandleInformation, buf, buf.real_size, ReturnLength=ctypes.byref(size_needed))
    handle_array = pfw_windows.utils.resized_array(buf[0].Handles, buf[0].HandleCount, Handle)
...
def enumerate_handles_syswow64():
    size_needed = gdef.ULONG()
    # Should at least be sizeof(gdef.SYSTEM_HANDLE_INFORMATION)
    tmp_buffer = pfw_windows.utils.BUFFER(gdef.SYSTEM_HANDLE_INFORMATION64)()
    try:
        pfw_windows.syswow64.NtQuerySystemInformation_32_to_64(gdef.SystemHandleInformation, tmp_buffer, tmp_buffer.real_size, ReturnLength=ctypes.byref(size_needed))
    except WindowsError as e:
        pass
    size = size_needed.value + 0x1000 # In case we have some more handle created
    buf = pfw_windows.utils.BUFFER(gdef.SYSTEM_HANDLE_INFORMATION64)(size=size)
    size_needed.value = 0
    pfw_windows.syswow64.NtQuerySystemInformation_32_to_64(gdef.SystemHandleInformation, buf, buf.real_size, ReturnLength=ctypes.byref(size_needed))
    handle_array = pfw_windows.utils.resized_array(buf[0].Handles, buf[0].HandleCount, HandleWow64)
    return list(handle_array)
...
def enumerate_type():
        ...
        with pfw_windows.current_process.allocated_memory(size, gdef.PAGE_READWRITE) as buffer_base:
