...
class ServiceManager(utils.AutoHandle):
    ...
    def _get_handle(self):
        return pfw_windows.winproxy.OpenSCManagerW(dwDesiredAccess=gdef.MAXIMUM_ALLOWED)

    def open_service(self, name, access=gdef.MAXIMUM_ALLOWED):
        return pfw_windows.winproxy.OpenServiceW(self.handle, name, access) # Check service exists :)
    ...
    def get_service_display_name(self, name):
        ...
        pfw_windows.winproxy.GetServiceDisplayNameW(self.handle, name, result, size_needed)
        return result.value
    ...
    def _enumerate_services_generator(self):
        ...
        counter =  gdef.DWORD()
        try:
            pfw_windows.winproxy.EnumServicesStatusExW(self.handle, SC_ENUM_PROCESS_INFO, SERVICE_TYPE_ALL, SERVICE_STATE_ALL, None, 0, ctypes.byref(size_needed), ctypes.byref(nb_services), byref(counter), None)
        except WindowsError:
            pass
        ...
        while True:
            ...
            buffer = (BYTE * size)()
            try:
                pfw_windows.winproxy.EnumServicesStatusExW(self.handle, SC_ENUM_PROCESS_INFO, SERVICE_TYPE_ALL, SERVICE_STATE_ALL, buffer, size, ctypes.byref(size_needed), ctypes.byref(nb_services), byref(counter), None)
...
class Service(gdef.SC_HANDLE):
    ...
    def status(self):
        ...
        :type: :class:`~pfw_windows.generated_def.winstructs.SERVICE_STATUS_PROCESS`
        """
        buffer = pfw_windows.utils.BUFFER(gdef.SERVICE_STATUS_PROCESS)()
        size_needed = gdef.DWORD()
        pfw_windows.winproxy.QueryServiceStatusEx(self, gdef.SC_STATUS_PROCESS_INFO, buffer.cast(gdef.LPBYTE), ctypes.sizeof(buffer), size_needed)
        return buffer[0]
    ...
    def process(self):
        ...
        :type: :class:`WinProcess <pfw_windows.winobject.process.WinProcess>` or ``None``
        """
        pid = self.status.dwProcessId
        if not pid:
            return None
        l = pfw_windows.WinProcess(pid=pid)
        return l
    ...
    def security_descriptor(self):
        ...
        :type: :class:`~pfw_windows.security.SecurityDescriptor`
        """
    ...
    def start(self, args=None):
        ...
        nbelt = 0
        if args is not None:
            if isinstance(args, pfw_windows.pycompat.anybuff):
                args = [args]
            nbelt = len(args)
            args = (gdef.LPWSTR * (nbelt))(*args)
        return pfw_windows.winproxy.StartServiceW(self, nbelt, args)

    def stop(self):
        """Stop the service"""
        status = SERVICE_STATUS()
        pfw_windows.winproxy.ControlService(self, gdef.SERVICE_CONTROL_STOP, status)
        return status
    ...
    def __del__(self):
        return pfw_windows.winproxy.CloseServiceHandle(self)
