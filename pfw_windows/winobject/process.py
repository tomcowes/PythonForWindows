...
from pfw_windows import injection
from pfw_windows import native_exec
from pfw_windows import pe_parse
from pfw_windows import winproxy
from pfw_windows import utils
from pfw_windows.dbgprint import dbgprint
from pfw_windows.generated_def.winstructs import *
from pfw_windows.generated_def.ntstatus import NtStatusException

from pfw_windows.winobject import exception
from pfw_windows.winobject import apisetmap
from pfw_windows.winobject import token
from pfw_windows import security

from pfw_windows.pycompat import raw_encode, raw_decode, basestring
...
class Process(utils.AutoHandle):
    ...
    def bitness(self):
		...
		"""
        if pfw_windows.system.bitness == 32:
            return 32
        if self.is_wow_64:
            return 32
        return 64
    ...
    def limited_handle(self):
        if pfw_windows.system.version[0] <= 5:
    ...
    def ppid(self):
        ...
        :type: :class:`int`
		"""
        if pfw_windows.current_process.bitness == 32 and self.bitness == 64:
            xtype = pfw_windows.remotectypes.transform_type_to_remote64bits(PROCESS_BASIC_INFORMATION)
            # Fuck-it <3
            data = (ctypes.c_char * ctypes.sizeof(xtype))()
            pfw_windows.syswow64.NtQueryInformationProcess_32_to_64(self.limited_handle, ProcessInformation=data, ProcessInformationLength=ctypes.sizeof(xtype))
            # Map a remote64bits(PROCESS_BASIC_INFORMATION) at the address of 'data'
            x = xtype(ctypes.addressof(data), pfw_windows.current_process)
        else:
            information_type = 0
    ...
    def threads(self):
		...
		"""
        owner_pid = self.pid
        return [WinThread._from_THREADENTRY32(th, owner=self) for th in pfw_windows.system.enumerate_threads_generator() if th.th32OwnerProcessID == owner_pid]
    ...
    def virtual_protect(self, addr, size, protect, old_protect=None):
        ...
        if pfw_windows.current_process.bitness == 32 and self.bitness == 64:
            ...
            return pfw_windows.syswow64.NtProtectVirtualMemory_32_to_64(self.handle, addr, size, protect, xold_protect)
        else:
    ...
    def query_memory(self, addr):
        ...
        :rtype: :class:`~pfw_windows.generated_def.winstructs.MEMORY_BASIC_INFORMATION`
		"""
        if pfw_windows.current_process.bitness == 32 and self.bitness == 64:
            res = MEMORY_BASIC_INFORMATION64()
            try:
                v = pfw_windows.syswow64.NtQueryVirtualMemory_32_to_64(ProcessHandle=self.handle, BaseAddress=addr, MemoryInformationClass=MemoryBasicInformation, MemoryInformation=res)
        ...
        res = info_type[pfw_windows.current_process.bitness]()
    ...
    def memory_state(self):
        ...
        :yield: :class:`~pfw_windows.generated_def.winstructs.MEMORY_BASIC_INFORMATION`
		"""
        addr = 0
        res = []
        while True:
    ...
    def query_working_set(self):
        if self.bitness == 64 or pfw_windows.current_process.bitness == 64:
        ...
        try:
            pfw_windows.winproxy.QueryWorkingSet(self.handle, ctypes.byref(dummy), ctypes.sizeof(dummy))
        ...
        for i in range(10):
            ...
            try:
                if pfw_windows.current_process.bitness == 32 and self.bitness == 64:
                    pfw_windows.syswow64.NtQueryVirtualMemory_32_to_64(self.handle, 0, MemoryWorkingSetList, res)
                else:
                    pfw_windows.winproxy.QueryWorkingSet(self.handle, ctypes.byref(res), ctypes.sizeof(res))
            except WindowsError as e:
                if e.winerror != 24:
                    raise
                dummy.NumberOfEntries = res.NumberOfEntries
                continue
            except pfw_windows.generated_def.ntstatus.NtStatusException as e:
    ...
    def query_working_setex(self, addresses):
        ...
        if pfw_windows.current_process.bitness == 32 and self.bitness == 64:
            pfw_windows.syswow64.NtQueryVirtualMemory_32_to_64(self.handle, 0, MemoryWorkingSetListEx, info_array)
        else:
    ...
    def get_mapped_filename(self, addr):
        ...
        if  pfw_windows.current_process.bitness == 32 and self.bitness == 64:
             ...
             try:
                pfw_windows.syswow64.NtQueryVirtualMemory_32_to_64(self.handle, addr, MemorySectionName, buffer, buffer_size, target_size)
             except NtStatusException as e:
             ...
             mapped_filename = remote_winstring(ctypes.addressof(buffer), pfw_windows.current_process)
    ...
    def memory_info(self):
        ...
        pfw_windows.winproxy.GetProcessMemoryInfo(self.limited_handle, cast_result, result.cb)
        return result
    ...
    def open_token(self, flags=MAXIMUM_ALLOWED):
        ...
        :returns: :class:`~pfw_windows.winobject.token.Token`
        """
        token_handle = HANDLE()
        winproxy.OpenProcessToken(self.limited_handle, flags, byref(token_handle))
        return token.Token(token_handle.value)

    token = property(open_token, doc="The process :class:`~pfw_windows.winobject.token.Token`")
    ...
    def get_security_descriptor(self,  query_sacl=False, flags=security.SecurityDescriptor.DEFAULT_SECURITY_INFORMATION):
        ...
        tmp_handle = pfw_windows.winproxy.OpenProcess(open_flags, 0, self.pid)
        try:
            return security.SecurityDescriptor.from_handle(tmp_handle, query_sacl=query_sacl, flags=flags, obj_type="process")
        finally:
            pfw_windows.winproxy.CloseHandle(tmp_handle)
    ...
    def handles(self):
        pid = self.pid
        return [h for h in pfw_windows.system.handles if h.dwProcessId == pid]
...
class Thread(utils.AutoHandle):
    ...
    _TOKEN_PROPERTY_DOC = """The thread :class:`~pfw_windows.winobject.token.Token`
...
class CurrentThread(Thread):
    ...
    def owner(self):
		...
		"""
        return pfw_windows.current_process
...
class CurrentProcess(Process):
    ...
    def execute(self, code, parameter=0):
        ...
        f = pfw_windows.native_exec.create_function(code, [PVOID, PVOID])
        return f(parameter)
    ...
    def peb_syswow(self):
        ...
        return pfw_windows.syswow64.get_current_process_syswow_peb()
...
class WinThread(Thread):
    ...
    def owner_pid(self):
        ...
        pfw_windows.winproxy.NtQueryInformationThread(self.handle, ThreadBasicInformation, byref(res), ctypes.sizeof(res))
    ...
    def context(self):
        ...
        :type: :class:`pfw_windows.exception.ECONTEXT32` or  :class:`pfw_windows.exception.ECONTEXT64` or :class:`pfw_windows.exception.ECONTEXTWOW64`
		"""
        if self.owner.bitness == 32 and pfw_windows.current_process.bitness == 64:
            # Wow64
        ...
        if self.owner.bitness == 64 and pfw_windows.current_process.bitness == 32:
            x = exception.ECONTEXT64.new_aligned()
            x.ContextFlags = CONTEXT_ALL
            pfw_windows.syswow64.NtGetContextThread_32_to_64(self.handle, x)
            return x
    ...
    def context_syswow(self):
        ...
        :type:  :class:`pfw_windows.exception.ECONTEXT64`
		"""
    ...
    def set_context(self, context):
        ...
        if self.owner.bitness == pfw_windows.current_process.bitness:
            return winproxy.SetThreadContext(self.handle, context)
        if pfw_windows.current_process.bitness == 64 and self.owner.bitness == 32:
            return winproxy.Wow64SetThreadContext(self.handle, context)
        return pfw_windows.syswow64.NtSetContextThread_32_to_64(self.handle, ctypes.byref(context))
    ...
    def start_address(self):
        """The start address of the thread
            ...
            :type: :class:`int`
		"""
        if pfw_windows.current_process.bitness == 32 and self.owner.bitness == 64:
            res = ULONGLONG()
            pfw_windows.syswow64.NtQueryInformationThread_32_to_64(self.handle, ThreadQuerySetWin32StartAddress, byref(res), ctypes.sizeof(res))
            return res.value
        res_size = max(self.owner.bitness, pfw_windows.current_process.bitness)
        if res_size == 32:
            res = ULONG()
        else:
            res = ULONGLONG()
    ...
    def _get_principal_teb_addr(self):
        ...
        if pfw_windows.current_process.is_wow_64:
            ...
            pfw_windows.syswow64.NtQueryInformationThread_32_to_64(self.handle, ThreadBasicInformation, struct_address, ressize)
            return restype(struct_address, pfw_windows.current_process).TebBaseAddress
    ...
    def _get_thread_id_manual(handle):
        ...
        res = THREAD_BASIC_INFORMATION()
        pfw_windows.winproxy.NtQueryInformationThread(handle, ThreadBasicInformation, byref(res), ctypes.sizeof(res))
...
class WinProcess(Process):
    ...
    def _from_handle(handle):
        ...
        #proc = [p for p in pfw_windows.system.processes if p.pid == pid][0]
        #proc._handle = handle
    ...
    def create_thread(self, addr, param):
        ...
        if pfw_windows.current_process.bitness == 32 and self.bitness == 64:
            ...
            pfw_windows.syswow64.NtCreateThreadEx_32_to_64(ThreadHandle=byref(thread_handle) ,ProcessHandle=self.handle, lpStartAddress=addr, lpParameter=param)
    ...
    def load_library(self, dll_path):
        ...
        :rtype: :class:`LoadedModule`
        """
        dllbase = pfw_windows.injection.load_dll_in_remote_process(self, dll_path)
        return [m for m in self.peb.modules if m.baseaddr == dllbase][0]
    ...
    def peb_addr(self):
		...
		"""
        if pfw_windows.current_process.bitness == 32 and self.bitness == 64:
            x = pfw_windows.remotectypes.transform_type_to_remote64bits(PROCESS_BASIC_INFORMATION)
            # Fuck-it <3
            data = (ctypes.c_char * ctypes.sizeof(x))()
            pfw_windows.syswow64.NtQueryInformationProcess_32_to_64(self.handle, ProcessInformation=data, ProcessInformationLength=ctypes.sizeof(x))
            peb_offset = x.PebBaseAddress.offset
            peb_addr = struct.unpack("<Q", data[x.PebBaseAddress.offset: x.PebBaseAddress.offset+8])[0]
        elif pfw_windows.current_process.bitness == 64 and self.bitness == 32:
            information_type = 26
            y = ULONGLONG()
    ...
    def peb(self):
        ...
        if pfw_windows.current_process.bitness == 64 and self.bitness == 32:
    ...
    def peb_syswow(self):
        ...
        if pfw_windows.current_process.bitness == 64:
            ...
            return RemotePEB64(self.peb_syswow_addr, pfw_windows.syswow64.ReadSyswow64Process(self))
...
class LoadedModule(LDR_DATA_TABLE_ENTRY):
    ...
    def pe(self):
        ...
        :type: :class:`pfw_windows.pe_parse.PEFile`
		"""
...
class PEB(gdef.PEB):
    ...
    def exe(self):
        ...
        :type: :class:`pfw_windows.pe_parse.PEFile`
        """
        return pfw_windows.pe_parse.GetPEFile(self.ImageBaseAddress)

    @property
    def imagepath(self):
        """The ImagePathName of the PEB

        :type: :class:`~pfw_windows.generated_def.winstructs.LSA_UNICODE_STRING`
		"""
    ...
    def environment(self):
        # TODO: Tests
        return self._extract_environment(self.ProcessParameters.contents.Environment, pfw_windows.current_process)

    @property
    def apisetmap(self):
        """The :class:`~pfw_windows.winobject.apisetmap.ApiSetMap` of the process

        :rtype: A subclass of :class:`~pfw_windows.winobject.apisetmap.ApiSetMap`
        ...
        """
        if pfw_windows.system.version < (6,2):
...
class EPSAPI_WORKING_SET_EX_INFORMATION(ctypes.Structure):
    _fields_ = pfw_windows.utils.transform_ctypes_fields(PSAPI_WORKING_SET_EX_INFORMATION, {"VirtualAttributes": EPSAPI_WORKING_SET_EX_BLOCK})

class EPSAPI_WORKING_SET_EX_INFORMATION32(ctypes.Structure):
    _fields_ = pfw_windows.utils.transform_ctypes_fields(PSAPI_WORKING_SET_EX_INFORMATION32, {"VirtualAttributes": EPSAPI_WORKING_SET_EX_BLOCK32})

class EPSAPI_WORKING_SET_EX_INFORMATION64(ctypes.Structure):
    _fields_ = pfw_windows.utils.transform_ctypes_fields(PSAPI_WORKING_SET_EX_INFORMATION64, {"VirtualAttributes": EPSAPI_WORKING_SET_EX_BLOCK64})
