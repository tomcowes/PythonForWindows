...
from pfw_windows.pycompat import basestring, int_types, is_py3
...
class PyHKey(object):
    ...
    def values(self):
        ...
        with ExpectWindowsError(259):
            for i in itertools.count():
                ...
                databuffer = pfw_windows.utils.BUFFER(gdef.BYTE, nbelt=datasize.value)()
    ...
    def get(self, value_name):
        ...
        while True:
            buffer = pfw_windows.utils.BUFFER(gdef.BYTE, nbelt=size.value)()
            try:
    ...
    def set(self, name, value, type=None):
        ...
        if isinstance(buffer, bytes): # Should not be unicode at this point
            buffer = pfw_windows.utils.BUFFER(gdef.BYTE).from_buffer_copy(buffer)
    ...
    def delete(self):
        ...
        try:
            pfw_windows.winproxy.RegDeleteKeyExW(self.surkey.phkey, self.name, self.sam, 0)
        except WindowsError as e:
            raise WindowsError(e.winerror, "Could not delete registry key <{0}> ({1})".format(self.fullname, e.strerror))
        return None

    def empty(self):
        pfw_windows.winproxy.RegDeleteTreeW(self.phkey, None)
