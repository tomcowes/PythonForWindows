...
exception_name_by_value = dict([(x, x) for x in [getattr(pfw_windows.generated_def.windef, name) for name in exception_type]])
...
class EEXCEPTION_RECORDBase(object):
        ...
        def ExceptionCode(self):
            ...
            return exception_name_by_value.get(real_code, pfw_windows.generated_def.windef.Flag("UNKNOW_EXCEPTION", real_code))
...
class EEXCEPTION_DEBUG_INFO32(ctypes.Structure):
    """Enhanced Debug info"""
    _fields_ = pfw_windows.utils.transform_ctypes_fields(EXCEPTION_DEBUG_INFO, {"ExceptionRecord": EEXCEPTION_RECORD32})
...
class EEXCEPTION_DEBUG_INFO64(ctypes.Structure):
    """Enhanced Debug info"""
    _fields_ = pfw_windows.utils.transform_ctypes_fields(EXCEPTION_DEBUG_INFO, {"ExceptionRecord": EEXCEPTION_RECORD64})
...
class VectoredExceptionHandler(object):
    ...
    def __enter__(self):
        self.value = pfw_windows.winproxy.AddVectoredExceptionHandler(self.pos, self.handler)
        return self

    def __exit__(self, exc_type, exc_value, traceback):
        pfw_windows.winproxy.RemoveVectoredExceptionHandler(self.value)
        return False
...
class DumpContextOnException(VectoredExceptionHandler):
        ...
        def print_context_result(self, exception_pointers):
            ...
            sys.stdout.flush()
            if self.exit:
                pfw_windows.current_process.exit()
            return 0
