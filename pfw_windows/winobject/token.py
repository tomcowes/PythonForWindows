...
# import pfw_windows.security # at the end of this file (loop import)
...
class TokenGroups(gdef.TOKEN_GROUPS):
    @property
    def _groups(self):
        return pfw_windows.utils.resized_array(self.Groups, self.GroupCount)

    @property
    def sids_and_attributes(self):
        """The sids and attributes of each group

        :type: [:class:`~pfw_windows.generated_def.winstructs.SID_AND_ATTRIBUTES`] - A list of :class:`~pfw_windows.generated_def.winstructs.SID_AND_ATTRIBUTES`
        """
    ...
    def sids(self):
        ...
        :type: [:class:`~pfw_windows.generated_def.winstructs.PSID`] - A list of :class:`~pfw_windows.generated_def.winstructs.PSID`
        """
...
class TokenPrivileges(gdef.TOKEN_PRIVILEGES):
    ...
    def _privileges(self):
        return pfw_windows.utils.resized_array(self.Privileges, self.PrivilegeCount)

    def all(self):
        """The list of all privileges

        :returns: [:class:`~pfw_windows.generated_def.winstructs.LUID_AND_ATTRIBUTES`] - A list of :class:`~pfw_windows.generated_def.winstructs.LUID_AND_ATTRIBUTES`
        """
...
class Token(utils.AutoHandle):
    ...
    The attributes only documented by a type are from the :class:`~pfw_windows.generated_def.winstructs.TOKEN_INFORMATION_CLASS`, such return values may be improved version of the structure.
    ...
    TokenUser = craft(gdef.TokenUser, gdef.TOKEN_USER) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_USER`
    ...
    TokenOwner = craft(gdef.TokenOwner, gdef.TOKEN_OWNER) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_OWNER`
    TokenPrimaryGroup = craft(gdef.TokenPrimaryGroup, gdef.TOKEN_PRIMARY_GROUP) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_PRIMARY_GROUP`
    TokenDefaultDacl = craft(gdef.TokenDefaultDacl, gdef.TOKEN_DEFAULT_DACL) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_DEFAULT_DACL`
    TokenSource = craft(gdef.TokenSource, gdef.TOKEN_SOURCE) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_SOURCE`
    TokenType = craft(gdef.TokenType, gdef.TOKEN_TYPE) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_TYPE`
    TokenImpersonationLevel = craft(gdef.TokenImpersonationLevel, gdef.SECURITY_IMPERSONATION_LEVEL) #: :class:`~pfw_windows.generated_def.winstructs.SECURITY_IMPERSONATION_LEVEL`
    TokenStatistics = craft(gdef.TokenStatistics, gdef.TOKEN_STATISTICS) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_STATISTICS`
    TokenRestrictedSids = craft(gdef.TokenRestrictedSids, TokenGroupsType) #: :class:`~pfw_windows.generated_def.winstructs.TokenGroups`
    TokenSessionId = craft(gdef.TokenSessionId, gdef.DWORD) #: :class:`~pfw_windows.generated_def.winstructs.DWORD`
    TokenGroupsAndPrivileges = craft(gdef.TokenGroupsAndPrivileges, gdef.TOKEN_GROUPS_AND_PRIVILEGES) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_GROUPS_AND_PRIVILEGES`
    # TokenSessionReference = craft(gdef.TokenSessionReference, ???) # Reserved.
    TokenSandBoxInert = craft(gdef.TokenSandBoxInert, gdef.DWORD) #: :class:`~pfw_windows.generated_def.winstructs.DWORD`
    # TokenAuditPolicy = craft(gdef.TokenAuditPolicy, ???) # Reserved.
    TokenOrigin = craft(gdef.TokenOrigin, gdef.TOKEN_ORIGIN) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_ORIGIN`
    TokenElevationType = craft(gdef.TokenElevationType, gdef.TOKEN_ELEVATION_TYPE) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_ELEVATION_TYPE`
    TokenLinkedToken = craft(gdef.TokenLinkedToken, gdef.TOKEN_LINKED_TOKEN) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_LINKED_TOKEN`
    TokenElevation = craft(gdef.TokenElevation, gdef.TOKEN_ELEVATION) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_ELEVATION`
    TokenHasRestrictions = craft(gdef.TokenHasRestrictions, gdef.DWORD) #: :class:`~pfw_windows.generated_def.winstructs.DWORD`
    TokenAccessInformation = craft(gdef.TokenAccessInformation, gdef.TOKEN_ACCESS_INFORMATION) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_ACCESS_INFORMATION`
    TokenVirtualizationAllowed = craft(gdef.TokenVirtualizationAllowed, gdef.DWORD) #: :class:`~pfw_windows.generated_def.winstructs.DWORD`
    TokenVirtualizationEnabled = craft(gdef.TokenVirtualizationEnabled, gdef.DWORD) #: :class:`~pfw_windows.generated_def.winstructs.DWORD`
    TokenIntegrityLevel = craft(gdef.TokenIntegrityLevel, gdef.TOKEN_MANDATORY_LABEL) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_MANDATORY_LABEL`
    TokenUIAccess = craft(gdef.TokenUIAccess, gdef.DWORD) #: :class:`~pfw_windows.generated_def.winstructs.DWORD`
    TokenMandatoryPolicy = craft(gdef.TokenMandatoryPolicy, gdef.TOKEN_MANDATORY_POLICY) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_MANDATORY_POLICY`
    TokenLogonSid = craft(gdef.TokenLogonSid, TokenGroupsType) #: :class:`TokenGroups`
    TokenIsAppContainer = craft(gdef.TokenIsAppContainer, gdef.DWORD) #: :class:`~pfw_windows.generated_def.winstructs.DWORD`
    TokenCapabilities = craft(gdef.TokenCapabilities, TokenGroupsType) #: :class:`TokenGroups`
    TokenAppContainerSid = craft(gdef.TokenAppContainerSid, gdef.TOKEN_APPCONTAINER_INFORMATION) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_APPCONTAINER_INFORMATION`
    TokenAppContainerNumber = craft(gdef.TokenAppContainerNumber, gdef.DWORD) #: :class:`~pfw_windows.generated_def.winstructs.DWORD`
    TokenUserClaimAttributes = craft(gdef.TokenUserClaimAttributes, gdef.CLAIM_SECURITY_ATTRIBUTES_INFORMATION) #: :class:`~pfw_windows.generated_def.winstructs.CLAIM_SECURITY_ATTRIBUTES_INFORMATION`
    TokenDeviceClaimAttributes = craft(gdef.TokenDeviceClaimAttributes, gdef.CLAIM_SECURITY_ATTRIBUTES_INFORMATION) #: :class:`~pfw_windows.generated_def.winstructs.CLAIM_SECURITY_ATTRIBUTES_INFORMATION`
    ...
    TokenRestrictedDeviceGroups = craft(gdef.TokenRestrictedDeviceGroups, gdef.TOKEN_GROUPS) #: :class:`~pfw_windows.generated_def.winstructs.TOKEN_GROUPS`
    # Reserved.
    ...
    TokenProcessTrustLevel = craft(gdef.TokenProcessTrustLevel, gdef.PSID) #: :class:`~pfw_windows.generated_def.winstructs.PSID`
    ...
    def user(self):
        ...
        :type: :class:`~pfw_windows.generated_def.winstructs.PSID`
        """
    ...
    def _user_and_computer_name(self):
        return pfw_windows.utils.lookup_sid(self.user)
    ...
    def default_dacl(self):
        ...
        :type: :class:`pfw_windows.security.Acl`
        """
        return self.get_token_infomations(gdef.TokenDefaultDacl, pfw_windows.security.PAcl)[0]
    ...
    def impersonation_level(self):
        ...
        :type: :class:`int` -- Enum value from :class:`~pfw_windows.generated_def.winstructs.SECURITY_IMPERSONATION_LEVEL`
        """
        try:
    ...
    def elevation_type(self):
        ...
        :type: :class:`int` -- Enum value from :class:`~pfw_windows.generated_def.winstructs.TOKEN_ELEVATION_TYPE`
        """
    ...
    def integrity_level(self):
        ...
        :type: :class:`pfw_windows.generated_def.winstructs.SID_AND_ATTRIBUTES`
        """
    ...
    def duplicate(self, access_rigth=gdef.MAXIMUM_ALLOWED, attributes=None, type=None, impersonation_level=None):
        ...
        :param impersonation_level: The :class:`~pfw_windows.generated_def.winstructs.SECURITY_IMPERSONATION_LEVEL` for a ``TokenImpersonation(0x2L)``:
    ...
    def adjust_privileges(self, privileges):
        ...
        :param privileges: :class:`~pfw_windows.generated_def.winstructs.TOKEN_PRIVILEGES` (or subclass as :class:`TokenPrivileges`). To easily update your token privileges use the result of :data:`privileges`.
