...
class ThreadState(threading.local):
    def __init__(self): # Called once per thread
        self.allocator =  pfw_windows.native_exec.native_function.CustomAllocator()
...
def generate_64bits_execution_stub_from_syswow(x64shellcode):
    ...
    current_process = pfw_windows.current_process
...
def try_generate_stub_target(shellcode, argument_buffer, target, errcheck=None):
    if not pfw_windows.current_process.is_wow_64:
...
def get_current_process_syswow_peb():
    ...
    return pfw_windows.winobject.process.RemotePEB64(peb_addr, CurrentProcessReadSyswow())
...
class Syswow64ApiProxy(object):
    ...
    def __call__(self, python_proxy):
        if not pfw_windows.winproxy.is_implemented(self.winproxy_function):
            return None
        ...
        def force_resolution():
            ...
            try:
                ...
                raise pfw_windows.winproxy.ExportNotFound(self.winproxy_function.__name__, "SysWow[ntdll64]")
...
def NtGetContextThread_32_to_64(hThread, lpContext):
    if type(lpContext) == pfw_windows.winobject.exception.ECONTEXT64:
