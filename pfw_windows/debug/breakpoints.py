...
class FunctionParamDumpBPAbstract(object):
    ...
    def extract_arguments_32bits(self, cproc, cthread):
        x = pfw_windows.debug.X86ArgumentRetriever()
        res = OrderedDict()
        for i, (name, type) in enumerate(zip(self.target_params, self.target_args)):
            value = x.get_arg(i, cproc, cthread)
            rt = pfw_windows.remotectypes.transform_type_to_remote32bits(type)
            if issubclass(rt, pfw_windows.remotectypes.RemoteValue):
                t = rt(value, cproc)
            else:
                t = rt(value)
            # Will fail in py3..
    ...
    def extract_arguments_64bits(self, cproc, cthread):
        x = pfw_windows.debug.X64ArgumentRetriever()
        res = OrderedDict()
        for i, (name, type) in enumerate(zip(self.target_params, self.target_args)):
            value = x.get_arg(i, cproc, cthread)
            rt = pfw_windows.remotectypes.transform_type_to_remote64bits(type)
            if issubclass(rt, pfw_windows.remotectypes.RemoteValue):
                t = rt(value, cproc)
            else:
                t = rt(value)
    ...
    def extract_arguments(self, cproc, cthread):
        ...
        if cthread.context.SegCs == pfw_windows.syswow64.CS_32bits:
            return self.extract_arguments_32bits(cproc, cthread)
        return self.extract_arguments_64bits(cproc, cthread)
    ...
    def arguments(self, dbg):
        "TEST PARAM DICT"
        if pfw_windows.current_process.bitness == 32:
            extractor = pfw_windows.debug.X86ArgumentRetriever()
        elif dbg.current_process.bitness == 64:
            extractor = pfw_windows.debug.X64ArgumentRetriever()
        elif dbg.current_thread.context.SegCs == pfw_windows.syswow64.CS_32bits:
            extractor = pfw_windows.debug.X86ArgumentRetriever()
        else:
            extractor = pfw_windows.debug.X64ArgumentRetriever()
...
class FunctionParameterProxy(object):
    ...
    def __getitem__(self, x):
        ...
        rt = pfw_windows.remotectypes.transform_type_to_remote32bits(argtype)
        if issubclass(rt, pfw_windows.remotectypes.RemoteValue):
...
class FunctionBP(FunctionCallBP, FunctionParamDumpBP):
    """A breakpoint that accepts a function from :mod:`pfw_windows.winproxy` and able to:
