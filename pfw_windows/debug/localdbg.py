...
import pfw_windows
import pfw_windows.winobject.exception as winexception

from pfw_windows import winproxy
from pfw_windows.generated_def import windef
...
class FakeDebuggerCurrentThread(object):
    ...
    @property
    def tid(self):
        return pfw_windows.current_thread.tid
...
class LocalDebugger(object):
    ...
    def __init__(self):
        ...
        self.hxbp_info = None
        self.code = pfw_windows.native_exec.create_function(b"\xcc\xc3", [PVOID])
        self.veh_depth = 0
        self.current_exception = None
        self.exceptions_stack = [None]
        self.current_process =  pfw_windows.current_process
    ...
    def _pass_breakpoint(self, addr, single_step):
        with pfw_windows.utils.VirtualProtected(addr, 1, PAGE_EXECUTE_READWRITE):
            pfw_windows.current_process.write_memory(addr, self._memory_save[addr])
        self._reput_breakpoint[pfw_windows.current_thread.tid] = self.breakpoints[addr], single_step
    ...
    def _local_resolve(self, addr):
        ...
        dll, api = addr.split("!")
        dll = dll.lower()
        modules = {m.name[:-len(".dll")] if m.name.endswith(".dll") else m.name : m for m in pfw_windows.current_process.peb.modules}
        mod = None
        if dll in modules:
            mod = [modules[dll]]
        if not mod:
    ...
    def handle_exception(self, exc):
        ...
        if exp_code == EXCEPTION_SINGLE_STEP and pfw_windows.current_thread.tid in self._reput_breakpoint:
            bp, single_step = self._reput_breakpoint[pfw_windows.current_thread.tid]
            self._memory_save[bp._addr] = pfw_windows.current_process.read_memory(bp._addr, 1)
            with pfw_windows.utils.VirtualProtected(bp._addr, 1, PAGE_EXECUTE_READWRITE):
                pfw_windows.current_process.write_memory(bp._addr, b"\xcc")
            del self._reput_breakpoint[pfw_windows.current_thread.tid]
            if single_step:
                return self.on_exception(exc)
            return windef.EXCEPTION_CONTINUE_EXECUTION
        elif exp_code == EXCEPTION_SINGLE_STEP and exp_addr in self._hxbp_breakpoint[pfw_windows.current_thread.tid]:
            res = self._hxbp_breakpoint[pfw_windows.current_thread.tid][exp_addr].trigger(self, exc)
    ...
    def del_bp(self, bp, targets=None):
        ...
        if bp.type == STANDARD_BP:
            with pfw_windows.utils.VirtualProtected(bp.addr, 1, PAGE_EXECUTE_READWRITE):
                pfw_windows.current_process.write_memory(bp.addr, self._memory_save[bp.addr])
            del self._memory_save[bp.addr]
            del self.breakpoints[bp.addr]
            return
        if bp.type == HARDWARE_EXEC_BP:
            threads_by_tid = {t.tid: t for t in pfw_windows.current_process.threads}
            for tid in self._hxbp_breakpoint:
                if bp.addr in self._hxbp_breakpoint[tid] and self._hxbp_breakpoint[tid][bp.addr] == bp:
                    if tid == pfw_windows.current_thread.tid:
    ...
    def add_bp(self, bp, target=None):
        ...
        if bp.type != STANDARD_BP:
            raise NotImplementedError("Unknow BP type {0}".format(bp.type))
        if target not in [None, pfw_windows.current_process]:
        ...
        self._memory_save[addr] = pfw_windows.current_process.read_memory(addr, 1)
        with pfw_windows.utils.VirtualProtected(addr, 1, PAGE_EXECUTE_READWRITE):
            pfw_windows.current_process.write_memory(addr, b"\xcc")
        return
    ...
    def add_bp_hxbp(self, bp, targets=None):
        ...
        if targets is None:
            targets = pfw_windows.current_process.threads
        for thread in targets:
            if thread.owner.pid != pfw_windows.current_process.pid:
                raise ValueError("Cannot add HXBP to target in remote process {0}".format(thread))
            if thread.tid == pfw_windows.current_thread.tid:
    ...
    def setup_hxbp_callback(self, exc):
        with self.NewCurrentException(exc):
            ...
            pfw_windows.current_process.write_memory(exp_addr, b"\x90")
    ...
    def setup_hxbp_self_thread(self, addr):
        ...
        with winexception.VectoredExceptionHandler(1, self.setup_hxbp_callback):
            ...
            if x is None:
                raise ValueError("Could not setup HXBP")
            pfw_windows.current_process.write_memory(x, b"\xcc")
        return
