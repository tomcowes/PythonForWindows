...
def set_dbghelp_path(path):
    ...
    """
    loaded_modules =  [m.name.lower() for m in pfw_windows.current_process.peb.modules]
    if os.path.isdir(path):
        path = os.path.join(path, str(pfw_windows.current_process.bitness), "dbghelp.dll")
...
class SymbolInfoBase(object):
    ...
    def tag(self):
        ...
        :type: :class:`~pfw_windows.generated_def.winstructs.SymTagEnum`
        """
...
class SymbolInfoA(gdef.SYMBOL_INFO, SymbolInfoBase):
    ...
    Exemple:
        ...
        >>> sh = pfw_windows.debug.symbols.VirtualSymbolHandler()
...
class SymbolType(object):
    ...
    def _get_type_info(self, typeinfo, ires=None):
        ...
        pfw_windows.winproxy.SymGetTypeInfo(self.resolver.handle, self.modbase, self._typeid, typeinfo, ctypes.byref(res))
        if ires is not None:
            return ires
        newres = res.value
        if isinstance(res, gdef.LPWSTR):
            pfw_windows.winproxy.LocalFree(res)
        return newres
...
class SymbolModule(gdef.IMAGEHLP_MODULE64):
    ...
    @property
    def type(self):
        """The type of module (:class:`~pfw_windows.generated_def.winstructs.SYM_TYPE`),
        which can be one of:
...
class SymbolHandler(object):
    ...
    def load_module(self, file_handle=None, path=None, name=None, addr=0, size=0, data=None, flags=0):
        ...
        if name is not None and file_handle is None and os.path.exists(path):
            try:
                f = open(path)
                file_handle = pfw_windows.utils.get_handle_from_file(f)
                path = name
            except Exception as e:
                pass
        # Expect a-string
        path = pfw_windows.pycompat.raw_encode(path)
        try:
    ...
    def modules(self):
        ...
        """
        res = []
        pfw_windows.winproxy.SymEnumerateModules64(self.handle, self.modules_aggregator, res)
    ...
    def symbol_and_displacement_from_address(self, addr):
        ...
        buff = pfw_windows.utils.BUFFER(SymbolInfo)(size=full_size)
        sym = buff[0]
    ...
    def symbol_from_name(self, name):
        ...
        sym.MaxNameLen  = max_len_size
        # Expect a-string
        name = pfw_windows.pycompat.raw_encode(name)
        pfw_windows.winproxy.SymFromName(self.handle, name, buff)
        sym.resolver = self
        sym.displacement = 0
        return sym
    ...
    def resolve(self, name_or_addr):
        ...
        if isinstance(name_or_addr, pfw_windows.pycompat.anybuff):
    ...
    def simple_aggregator(info, size, ctx):
        ...
        cpy = pfw_windows.utils.BUFFER(SymbolInfo)(size=fullsize)
    ...
    def search(self, mask, mod=0, tag=0, options=gdef.SYMSEARCH_ALLITEMS, callback=None):
        ...
        # Expect A-string
        mask = pfw_windows.pycompat.raw_encode(mask)
        pfw_windows.winproxy.SymSearch(self.handle, gdef.DWORD64(addr), 0, tag, mask, 0, callback, res, options)
        for sym in res:
            sym.resolver = self
            sym.displacement = 0
        return res
    ...
    def get_symbols(self, addr, callback=None):
        ...
        if callback is None:
            ...
            callback = ctypes.WINFUNCTYPE(gdef.BOOL, ctypes.POINTER(SymbolInfo), gdef.ULONG , ctypes.py_object)(callback)
        try:
            pfw_windows.winproxy.SymEnumSymbolsForAddr(self.handle, addr, callback, res)
        except WindowsError as e:
            if e.winerror == gdef.ERROR_MOD_NOT_FOUND:
                return []
            raise
    ...
    def get_type(self, name, mod=0):
        ...
        pfw_windows.winproxy.SymGetTypeFromName(self.handle, mod, name, buff)
...
class StackWalker(object):
    def __init__(self, resolver, process=None, thread=None, context=None):
        ...
        if pfw_windows.current_process.bitness == 32 and process.bitness == 64:
    ...
    def _stack_frame_generator(self):
        ...
        while True:
            try:
                pfw_windows.winproxy.StackWalkEx(machine,
...
class ProcessSymbolHandler(SymbolHandler):
    ...
    def load(self, name):
        ...
        Exemple:
            ...
            >>> sh = pfw_windows.debug.symbols.ProcessSymbolHandler(pfw_windows.test.pop_proc_64())
            <pfw_windows.debug.symbols.ProcessSymbolHandler object at 0x033A2C30>
            >>> sh
            <pfw_windows.debug.symbols.ProcessSymbolHandler object at 0x033A2C30>
    ...
    def refresh(self):
        ...
        Exemple:
            ...
            [<SymbolModule name="notepad" type=SymDeferred pdb="" addr=0x7ff772b80000>,
                ...
                ...]
        """
        return pfw_windows.winproxy.SymRefreshModuleList(self.handle)
...
class SymbolEngine(object):
    ...
    Its instance can be accessed using ``pfw_windows.debug.symbols.engine``

    Exemple:

        >>> pfw_windows.debug.symbols.engine.options
        6L
        >>> pfw_windows.debug.symbols.engine.options = gdef.SYMOPT_UNDNAME
        >>> pfw_windows.debug.symbols.engine.options
        2L
    ...
    def set_options(self, options):
        ...
        return pfw_windows.winproxy.SymSetOptions(options)

    def get_options(self):
        return pfw_windows.winproxy.SymGetOptions()
...
TST_TYPE_RES_TYPE = {
    ...
    gdef.TI_GET_VALUE: pfw_windows.com.Variant,
}
