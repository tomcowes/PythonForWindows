...
from pfw_windows.winobject.process import WinProcess, WinThread
...
#from pfw_windows.syswow64 import CS_32bits
...
class Debugger(object):
    ...
    def detach(self, target=None):
        ...
        if target is None:
            ...
            if not targets:
                ...
                if self.target:
                    ...
                    pfw_windows.winproxy.DebugActiveProcessStop(tpid)
                return
        ...
        if self.target and target.pid == self.target.pid:
            self.target = None
        pfw_windows.winproxy.DebugActiveProcessStop(target.pid)
    ...
    def debug(cls, path, args=None, dwCreationFlags=0, show_windows=False):
        ...
        c = pfw_windows.utils.create_process(path, args=args, dwCreationFlags=dwCreationFlags, show_windows=show_windows)
        return cls(c)
    ...
    def _add_exe_to_module_list(self, create_process_event):
        ...
        self._module_by_process[self.current_process.pid][exe_name] = pfw_windows.pe_parse.GetPEFile(create_process_event.lpBaseOfImage, self.current_process)
    ...
    def _compute_page_access_for_event(self, target, events):
        ...
        if events == set("X"):
            ...
            if target.bitness == 64:
                has_DEP = True
            elif pfw_windows.winproxy.is_implemented(pfw_windows.winproxy.GetProcessDEPPolicy):
                has_DEP = DWORD()
                permaned = LONG()
                pfw_windows.winproxy.GetProcessDEPPolicy(target.handle, has_DEP, permaned)
    ...
    def _get_loaded_dll(self, load_dll):
        name_sufix = ""
        pe = pfw_windows.pe_parse.GetPEFile(load_dll.lpBaseOfDll, self.current_process)
    ...
    def _handle_create_process(self, debug_event):
        ...
        cp_handle = pfw_windows.current_process.handle
    ...
    def _handle_load_dll(self, debug_event):
        ...
        self._module_by_process[self.current_process.pid][dll_name] = pfw_windows.pe_parse.GetPEFile(load_dll.lpBaseOfDll, self.current_process)
    ...
    def kill_on_exit(choice):
        ...
        """
        return pfw_windows.winproxy.DebugSetProcessKillOnExit(choice)
    ...
    def on_exception(self, exception):
        """Called on exception event other that known breakpoint or requested single step. ``exception`` is one of the following type:
                ...
                * :class:`pfw_windows.winobject.exception.EEXCEPTION_DEBUG_INFO32`
                * :class:`pfw_windows.winobject.exception.EEXCEPTION_DEBUG_INFO64`
