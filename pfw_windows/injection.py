...
from pfw_windows.generated_def import STATUS_THREAD_IS_TERMINATING
from pfw_windows.native_exec.nativeutils import GetProcAddress64, GetProcAddress32
...
def get_kernel32_dll_name():
    ...
    k32 = [m for m in pfw_windows.current_process.peb.modules if m.name == "kernel32.dll"]
    assert len(k32) == 1
...
def find_python_dll_to_inject(target_bitness):
    ...
    if pfw_windows.current_process.bitness == target_bitness:
        # We can inject our own DLL
        pymodules = [m for m in pfw_windows.current_process.peb.modules if m.name == pydll_name]
    ...
    assert pfw_windows.system.bitness == 64, "How can we have process of different bitness on 32b system ?"
    ...
    for base_key in "HKEY_LOCAL_MACHINE", "HKEY_CURRENT_USER":
        ...
        regbase = pfw_windows.system.registry(base_key, gdef.KEY_WOW64_64KEY | gdef.KEY_READ)
