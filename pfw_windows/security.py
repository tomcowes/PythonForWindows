...
from pfw_windows.winobject.token import Token, KNOW_INTEGRITY_LEVEL
...
class CallbackACE(MaskAndSidACE):
    ...
    @property
    def condition(self):
        buff = pfw_windows.utils.BUFFER(gdef.BYTE).from_buffer_copy(self.application_data)
        resstr = gdef.LPWSTR()
...
class ObjectRelatedACE(MaskAndSidACE):
    ...
    def object_type(self):
        ...
        :type: :class:`~pfw_windows.generated_def.winstructs.PSID` or ``None``
        """
...
class SecurityDescriptor(gdef.PSECURITY_DESCRIPTOR):
    ...
    @classmethod
    def create(cls):
        buff = pfw_windows.utils.BUFFER(gdef.SECURITY_DESCRIPTOR, 1)()
        winproxy.InitializeSecurityDescriptor(buff, gdef.SECURITY_DESCRIPTOR_REVISION)
        return buff.cast(pfw_windows.security.SecurityDescriptor)

    @property
    def control(self):
        """The security descriptor control

        :type: :class:`~pfw_windows.generated_def.winstructs.SECURITY_DESCRIPTOR_CONTROL`
        """
    ...
    def make_absolute(self):
        ...
        try:
            pfw_windows.winproxy.MakeAbsoluteSD(self, None, abs_descriptor_size,
                    None,
                    dacl_size,
                    None,
                    sacl_size,
        ...
        abs_sd = pfw_windows.utils.BUFFER(gdef.BYTE, abs_descriptor_size.value)().cast(SecurityDescriptor)
        # dacl = ctypes.create_string_buffer(dacl_size.value)
        dacl = pfw_windows.utils.BUFFER(gdef.BYTE, dacl_size.value)()
        sacl = pfw_windows.utils.BUFFER(gdef.BYTE, sacl_size.value)()
        owner = ctypes.create_string_buffer(owner_size.value)
        prim_groupe = ctypes.create_string_buffer(prim_groupe_size.value)

        pfw_windows.winproxy.MakeAbsoluteSD(self, abs_sd, abs_descriptor_size,
                dacl.cast(gdef.PACL),
                dacl_size,
                sacl.cast(gdef.PACL),
                sacl_size,
    ...
    def make_relative(self):
        ...
        try:
            pfw_windows.winproxy.MakeSelfRelativeSD(self, None, srsd_size)
        except WindowsError as e:
            if e.winerror != gdef.ERROR_INSUFFICIENT_BUFFER:
                raise

        srsd = pfw_windows.utils.BUFFER(gdef.BYTE, srsd_size.value)().cast(SecurityDescriptor)
        pfw_windows.winproxy.MakeSelfRelativeSD(self, srsd, srsd_size)
        return srsd
...
def explain_sid(sid):
    if sid is None:
        return "None"
    try:
        info = u"\\".join(pfw_windows.utils.lookup_sid(sid))
    except WindowsError as e:
        print(e)
...
def explain_mask(mask, sdtype):
    mapper = pfw_windows.security.SPECIFIC_ACCESS_RIGTH_BY_TYPE[sdtype]
    res = []
    for i in range(32): # sizeof ACCESS_MASK * 8
        v = mask & (1 << i)
        if v:
...
def explain_mandatory_label_ace(ace, sdtype):
    ...
    yield u"SID:"
    sid = ace.sid
    count = pfw_windows.winproxy.GetSidSubAuthorityCount(sid)
    integrity = pfw_windows.winproxy.GetSidSubAuthority(sid, count[0] - 1)[0]
