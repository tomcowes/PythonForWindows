...
from pfw_windows.crypto import DEFAULT_ENCODING

import pfw_windows.crypto.cryptmsg
...
class CryptObject(object):
    ...
    def __init__(self, filename, content_type=gdef.CERT_QUERY_CONTENT_FLAG_ALL):
        ...
        hMsg          = pfw_windows.crypto.CryptMessage()
    ...
    def signers_and_certs(self):
        ...
        :rtype: [(:class:`~pfw_windows.generated_def.winstructs.CMSG_SIGNER_INFO`, :class:`Certificate`)]

        .. note::

            :class:`~pfw_windows.generated_def.winstructs.CMSG_SIGNER_INFO` might be changed to a wrapping-subclass.
        """
...
class CertificateStore(gdef.HCERTSTORE):
    ...
    def certs(self):
        ...
        while True:
            ...
            ecert = pfw_windows.crypto.Certificate.from_pointer(cert)
    ...
    def _yolo(self):
        ...
        title = None
        pfw_windows.winproxy.CryptUIDlgViewContext(gdef.CERT_STORE_CTL_CONTEXT, x, None, title, 0, None)
        return x
...
def import_pfx(pfx, password=None, flags=gdef.CRYPT_USER_KEYSET | gdef.PKCS12_NO_PERSIST_KEY | gdef.PKCS12_ALWAYS_CNG_KSP):
    ...
    """
    if isinstance(pfx, pfw_windows.pycompat.anybuff) or isinstance(pfx,  bytearray):
...
class Certificate(gdef.CERT_CONTEXT):
    ...
    def view(self, title=None):
        return pfw_windows.winproxy.CryptUIDlgViewContext(gdef.CERT_STORE_CERTIFICATE_CONTEXT, ctypes.byref(self), None, title, 0, None)
    ...
    :type: [:class:`int` or :class:`~pfw_windows.generated_def.Flag`] -- A list of property ID
    """
    ...
    def get_property(self, prop):
        ...
        datasize = gdef.DWORD()
        pfw_windows.winproxy.CertGetCertificateContextProperty(self, prop, None, datasize)
        buf = (gdef.BYTE * datasize.value)()
        pfw_windows.winproxy.CertGetCertificateContextProperty(self, prop, buf, datasize)
    ...
    def from_file(cls, filename):
        ...
        with open(filename, "rb") as f:
            ...
            pcert = pfw_windows.winproxy.CertCreateCertificateContext(pfw_windows.crypto.DEFAULT_ENCODING, buf, len(data))
    ...
    def __eq__(self, other):
        ...
        return pfw_windows.winproxy.CertCompareCertificate(DEFAULT_ENCODING, self.pCertInfo, other.pCertInfo)
