...
def sign(cert, msg, detached_signature=False, algo=gdef.szOID_RSA_SHA256RSA):
    ...
    ByteBuffer = pfw_windows.utils.BUFFER(gdef.BYTE)

    result_buffer = ByteBuffer(nbelt=0x2000)
    result_size = gdef.DWORD(len(result_buffer))
    buff = ByteBuffer(*bytearray(msg))
    buff_pr = pfw_windows.utils.BUFFER(gdef.LPBYTE, nbelt=1)(buff)
    buff_size = gdef.DWORD(len(msg))
    try:
        pfw_windows.winproxy.CryptSignMessage(sign_para, False, 1, buff_pr, buff_size, result_buffer, result_size)
...
def verify_signature(cert, encoded_blob):
    ...
    verif_param.dwMsgEncodingType = pfw_windows.crypto.DEFAULT_ENCODING
    verif_param.hCryptProv = None
    # The public key used
    pubkey = cert.pCertInfo[0].SubjectPublicKeyInfo
    # Preparing in/out buffer/size
    signed_buffer = pfw_windows.utils.BUFFER(gdef.BYTE).from_buffer_copy(encoded_blob)
    decoded_buffer = pfw_windows.utils.BUFFER(gdef.BYTE).from_buffer_copy(encoded_blob)
