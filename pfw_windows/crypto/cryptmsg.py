...
class CryptMessage(gdef.HCRYPTMSG):
    ...
    def get_cert(self, index=0):
        ...
        """
        return pfw_windows.crypto.Certificate.from_buffer(self.get_raw_cert(index))
    ...
    def get_signer_data(self, index=0):
        ...
        :return: :class:`~pfw_windows.generated_def.winstructs.CMSG_SIGNER_INFO`
        """
        return self.get_param(gdef.CMSG_SIGNER_INFO_PARAM, index)

    @property
    def signers(self):
        """The list of :class:`~pfw_windows.generated_def.winstructs.CMSG_SIGNER_INFO` embed in the message"""
    ...
    def update(self, blob, final):
        ...
        if isinstance(blob, (pfw_windows.pycompat.anybuff, bytearray)):
            blob = pfw_windows.pycompat.raw_encode(blob)
            buffer = pfw_windows.utils.BUFFER(gdef.BYTE).from_buffer_copy(blob)
    ...
    def from_buffer(self, data):
        hmsg = winproxy.CryptMsgOpenToDecode(pfw_windows.crypto.DEFAULT_ENCODING, 0, 0, None, None, None)
