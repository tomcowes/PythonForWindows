import ctypes
import pfw_windows
import pfw_windows.hooks as hooks
import pfw_windows.utils as utils

from pfw_windows.generated_def.winstructs import *
from pfw_windows.utils import transform_ctypes_fields
...
def get_structure_transformer_for_target(target, targetbitness=None):
    current_bitness = pfw_windows.current_process.bitness
    if target is None:
...
def GetPEFile(baseaddr, target=None, force_bitness=None):
    ...
    """
    proc_bitness = pfw_windows.current_process.bitness
...
class IATEntry(ctypes.Structure):
    ...
    def set_hook(self, callback, types=None):
        ...
        :rtype: :class:`pfw_windows.hooks.IATHook`
...
class IMAGE_IMPORT_DESCRIPTOR(IMAGE_IMPORT_DESCRIPTOR): # TODO: use explicite name winstructs.IMAGE_IMPORT_DESCRIPTOR
    def get_INT(self, pe):
        ...
        if self.target is None and pe.bitness != pfw_windows.current_process.bitness:
            assert pfw_windows.current_process.bitness == 64 and pe.bitness == 32, "Mapped 64b PE in current process 32b not handled"
