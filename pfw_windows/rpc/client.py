...
import pfw_windows.alpc as alpc
...
class RPCClient(object):
    ...
    def __init__(self, port):
        self.alpc_client = alpc.AlpcClient(port) #: The :class:`pfw_windows.alpc.AlpcClient` used to communicate with the server
    ...
    def bind(self, IID_str, version=(1,0)):
        """Bind to the ``IID_str`` with the given ``version``
            ...
            :returns: :class:`pfw_windows.generated_def.IID`
        """
        IID = pfw_windows.com.IID.from_string(IID_str)
    ...
    def call(self, IID, method_offset, params, ipid=None):
        ...
        # pfw_windows.utils.sprint(ALPC_RPC_CALL.from_buffer_copy(response + "\x00" * 12))
    ...
    def _forge_call_request_in_view(self, interface_nb, method_offset, params, ipid=None):
        ...
        p = pfw_windows.alpc.AlpcMessage(0x2000)
        section = self.alpc_client.create_port_section(0x40000, 0, len(params))
        view = self.alpc_client.map_section(section[0], len(params))
        p.port_message.data = raw_request + pfw_windows.rpc.ndr.NdrLong.pack(len(params) + 0x200) + "\x00" * 40
        ...
        pfw_windows.current_process.write_memory(view.ViewBase, params) # Write NDR to view
        return p
