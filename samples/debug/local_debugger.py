...
class SingleSteppingDebugger(pfw_windows.debug.LocalDebugger):
    SINGLE_STEP_COUNT = 4
...
func = pfw_windows.native_exec.create_function(code, [PVOID])
print("Code addr = 0x{0:x}".format(func.code_addr))
# Create a thread that will infinite loop
t = pfw_windows.current_process.create_thread(func.code_addr, 0)
