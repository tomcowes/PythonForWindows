...
print(" is_signed: <{0}>".format(pfw_windows.wintrust.is_signed(TARGET_FILE)))
print(" check_signature: <{0}>".format(pfw_windows.wintrust.check_signature(TARGET_FILE)))

sign_info = pfw_windows.wintrust.full_signature_information(TARGET_FILE)
...
for module in pfw_windows.current_process.peb.modules[:5]:
    path = module.fullname
    is_signed =  pfw_windows.wintrust.is_signed(path)
    if is_signed:
        print("<{0}> : {1}".format(path, is_signed))
    else:
        sign_info = pfw_windows.wintrust.full_signature_information(path)
