...
if not pfw_windows.current_process.token.elevated:
...
sd = pfw_windows.security.SecurityDescriptor.from_filename(TARGET)
print("sacl = {0}".format(sd.sacl))

print("")
print("[NO-PRIV] Querying <{0}> SecurityDescriptor with SACL".format(TARGET))
try:
    sd = pfw_windows.security.SecurityDescriptor.from_filename(TARGET, query_sacl=True)
    print("sacl = {0}".format(sd.sacl))
...
try:
    pfw_windows.current_process.token.enable_privilege("SeSecurityPrivilege")
